...
class HelpCommandTests(unittest.IsolatedAsyncioTestCase):
    async def test_replies_with_command_descriptions_and_usage(self):
        message = SimpleNamespace(reply_text=AsyncMock())
        update = SimpleNamespace(effective_message=message)

        await help_command_handler(update, SimpleNamespace())

        message.reply_text.assert_awaited_once_with(messages.HELP_TEXT)
        self.assertIn("/mytelegramid", messages.HELP_TEXT)
        self.assertIn(
            messages.ADMIN_ADD_USER_USAGE_TEXT.removeprefix("Usage: "),
            messages.HELP_TEXT,
        )
        self.assertIn(
            messages.ADMIN_UPDATE_USER_USAGE_TEXT.removeprefix("Usage: "),
            messages.HELP_TEXT,
        )
        self.assertIn(
            messages.ADMIN_GIVE_KEY_USAGE_TEXT.removeprefix("Usage: "),
            messages.HELP_TEXT,
        )
        self.assertIn(
            messages.ADMIN_USERS_USAGE_TEXT.removeprefix("Usage: "),
            messages.HELP_TEXT,
        )
        self.assertIn(
            messages.ADMIN_KEY_HISTORY_USAGE_TEXT.removeprefix("Usage: "),
            messages.HELP_TEXT,
        )
        self.assertIn(
            messages.ADMIN_KEY_STATUS_USAGE_TEXT.removeprefix("Usage: "),
            messages.HELP_TEXT,
        )
        self.assertIn(
            messages.ADMIN_ACTIVATE_KEY_USAGE_TEXT.removeprefix("Usage: "),
            messages.HELP_TEXT,
        )
        self.assertIn(
            messages.ADMIN_DEACTIVATE_KEY_USAGE_TEXT.removeprefix("Usage: "),
            messages.HELP_TEXT,
        )
...
class GiveKeyCommandTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_rejects_non_admin(self):
        with (
            patch(
                "src.admin_commands.get_gym_member_by_telegram_user_id",
                return_value=build_member(100),
            ),
            patch("src.admin_commands.give_key_to_member") as give_key,
        ):
    ...
    async def test_reports_unregistered_target_member(self):
        with (
            ...
            patch(
                "src.admin_commands.give_key_to_member",
    ...
    async def test_gives_key_and_reports_target_member(self):
        ...
        with (
            ...
            patch("src.admin_commands.cancel_pending_handover") as cancel_handover,
        ):
            await give_key_command_handler(
                self.update,
                SimpleNamespace(args=["1", "200"]),
...
class AddUserCommandTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_rejects_non_admin(self):
        with (
            ...
            patch("src.admin_commands.add_gym_member") as add_member,
        ):
    ...
    async def test_reports_duplicate_telegram_user_id(self):
        with (
            ...
            patch(
                "src.admin_commands.add_gym_member",
    ...
    async def test_adds_user_with_optional_contact_data(self):
        ...
        with (
            ...
            patch("src.admin_commands.add_gym_member", return_value=member) as add_member,
        ):
            await add_user_command_handler(
                self.update,
                SimpleNamespace(
...
class UpdateUserArgumentTests(unittest.TestCase):
    def test_parses_options_in_any_order(self):
        telegram_user_id, updates = parse_update_user_arguments(
        ...
        for arguments in invalid_arguments:
            with self.subTest(arguments=arguments):
                with self.assertRaises(UpdateUserUsageError):
                    parse_update_user_arguments(arguments)
...
class UpdateUserCommandTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_rejects_non_admin(self):
        with (
            ...
            patch("src.admin_commands.update_gym_member") as update_member,
        ):
    ...
    async def test_updates_selected_fields_and_reports_actual_changes(self):
        ...
        with (
            ...
            patch(
                "src.admin_commands.update_gym_member",
...
class UsersCommandTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_rejects_non_admin(self):
        with (
            ...
            patch("src.admin_commands.get_gym_member_records") as get_members,
        ):
    ...
    async def test_reports_when_no_members_match(self):
        with (
            ...
            patch(
                "src.admin_commands.get_gym_member_records",
                return_value=[],
            ) as get_members,
        ):
            await users_command_handler(
    ...
    async def test_splits_large_results_between_telegram_messages(self):
        ...
        with (
            ...
            patch("src.admin_commands.get_gym_member_records", return_value=members),
            patch(
                "src.admin_commands.messages.gym_member_record_text",
...
class KeyHistoryCommandTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_rejects_non_admin(self):
        with (
            ...
            patch("src.admin_commands.get_key_history") as get_history,
        ):
    ...
    async def test_uses_key_id_and_default_limit_and_replies_with_history(self):
        ...
        with (
            ...
            patch(
                "src.admin_commands.get_key_history",
                return_value=[record],
            ) as get_history,
        ):
...
class KeyStatusCommandTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_rejects_non_admin(self):
        with (
            ...
            patch("src.admin_commands.get_key_status") as get_status,
        ):
    ...
    async def test_reports_missing_key(self):
        with (
            ...
            patch("src.admin_commands.get_key_status", return_value=None) as get_status,
        ):
    ...
    async def test_replies_with_complete_key_status(self):
        ...
        with (
            ...
            patch("src.admin_commands.get_key_status", return_value=status),
        ):
            await key_status_command_handler(self.update, SimpleNamespace(args=["1"]))
...
class SetKeyActiveCommandTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_activate_rejects_non_admin(self):
        with (
            ...
            patch("src.admin_commands.set_key_active") as set_active,
        ):
    ...
    async def test_activate_reports_missing_key(self):
        with (
            ...
            patch("src.admin_commands.set_key_active", return_value=False) as set_active,
        ):
    ...
    async def test_activate_sets_key_active(self):
        with (
            ...
            patch("src.admin_commands.set_key_active", return_value=True) as set_active,
        ):
