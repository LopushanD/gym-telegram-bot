...
def parse_update_user_arguments(arguments: list[str]) -> tuple[int, dict[str, str | int]]:
...
def split_record_texts(record_texts: list[str], separator: str) -> list[str]:
    replies = []
...
async def update_user_command_handler(update: Update, context) -> None:
    ...
    arguments = getattr(context, "args", [])
    try:
        telegram_user_id, updates = parse_update_user_arguments(arguments)
...
async def users_command_handler(update: Update, context) -> None:
    ...
    for reply in split_record_texts(member_texts, "\n\n"):
        await message.reply_text(reply)
...
async def key_history_command_handler(update: Update, context) -> None:
    ...
    for reply in split_record_texts(record_texts, "\n"):
        await message.reply_text(reply)
